...
class DiffusionModel3D():
    ...
    def convertMesh(self, mesh):
        # original tensors on CPU already
        self.vertices = mesh.verts.cpu()          # (V, 3)
        self.faces    = mesh.faces.cpu()          # (F, 3)
        self.colors   = self.getColors(mesh.vertex_channels)  # (V, 3)
    
        # --- new vectorised construction ---------------------------------
        v_np = self.vertices.numpy()
        f_np = self.faces.numpy()
        c_np = self.colors.numpy()
    
        self.mesh.vertices = v_np[f_np].astype(np.float32)    # (F, 3, 3)
        self.mesh.colors   = c_np[f_np].astype(np.float32)    # (F, 3, 3)
        # -----------------------------------------------------------------
    ...
    def calcTriangleAreas(self, v1_arr, v2_arr, v3_arr):
        # v*_arr are (F, 3) float32, so broadcasting is safe
        return 0.5 * np.linalg.norm(np.cross(v2_arr - v1_arr, v3_arr - v1_arr), axis=1)
